(function (global, factory) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (state === 3) {
        ...
        if (/^(?:Comment|Dialogue)\s*:/i.test(line)) {
          ...
          var key$1 = ref$1[1];
          var value$1 = ref$1[2];
          tree.events[key$1.toLowerCase()].push(
            parseDialogue(value$1, tree.events.format)
          );
        }
      }
    }
  ...
  function compileDrawing(rawCommands) {
    ...
    return assign(
      { instructions: instructions, d: toSVGPath(instructions) },
      getViewBox(commands)
    );
  }
  ...
  function compileTag(tag, key, presets) {
    ...
    if (key === "pos" || key === "org") {
      return value.length === 2
        ? ((obj = {}), (obj[key] = { x: value[0], y: value[1] }), obj)
        : null;
    }
    if (key === "move") {
      var x1 = value[0];
      ...
      if (dots) {
        ...
        return {
          clip: {
            ...
            scale: scale,
            drawing: drawing,
            dots: { x1: x1$1, y1: y1$1, x2: x2$1, y2: y2$1 },
          },
        };
      }
      return null;
  ...
  function compileText(ref) {
    ...
    return assign(
      { alignment: alignment, slices: slices },
      pos,
      org,
      move,
      fade,
  ...
  function parseStyleColor(color) {
    ...
    if (!Number.isNaN(num)) {
      ...
      var aabbggrr =
        min <= num && num <= max
          ? (
              "00000000" + (num < 0 ? num + 4294967296 : num).toString(16)
            ).slice(-8)
  ...
  function createClipPath(clip) {
    ...
    if (clip.inverse) {
      d +=
        "M0,0L0," + scale + "," + scale + "," + scale + "," + scale + ",0,0,0Z";
    }
    var id = "ASS-" + uuid();
    ...
    if (sx || sy) {
      ...
      for (var x$1 = Math.max(ox, sx - ox); x$1 < sx + ox; x$1++) {
        for (var y$1 = Math.max(oy, sy - oy); y$1 < sy + oy; y$1++) {
          arr.push(
            sc + " " + x$1 * pnx + "px " + y$1 * pny + "px " + blur + "px"
          );
        }
      }
      arr.push(
  ...
  function createTransform(tag) {
    return [
      ...
      "scale(" +
        (tag.p ? 1 : (tag.fscx || 100) / 100) +
        "," +
        (tag.p ? 1 : (tag.fscy || 100) / 100) +
        ")",
  ...
  function setTransformOrigin(dialogue) {
    ...
    for (var i = $div.childNodes.length - 1; i >= 0; i--) {
      ...
      if (node.dataset.hasRotate === "true") {
        ...
        node.style.cssText +=
          vendor.transform + "transform-origin:" + tox + "px " + toy + "px;";
      }
    }
  }
    ...
    return Object.keys(this.obj)
      .map(function (keyText) {
        return (
          keyText +
          "{" +
          Object.keys(this$1.obj[keyText])
            .map(function (prop) {
              return (
                "" +
                (vendor[prop] || "") +
    ...
    this.dialogues.forEach(function (dialogue) {
      ...
      if (move) {
        ...
        var values = [
          ...
          return (
            "translate(" +
            this$1.scale * (x - pos.x) +
            "px, " +
            this$1.scale * (y - pos.y) +
            "px)"
          );
        });
        diaKbl.setT({
      ...
      if (fade) {
        if (fade.type === "fad") {
          ...
          if (t1$1 < duration) {
            diaKbl.set(
              ((t1$1 / duration) * 100).toFixed(3) + "%",
              "opacity",
              1
            );
            if (t1$1 + t2$1 < duration) {
              diaKbl.set(
                (((duration - t2$1) / duration) * 100).toFixed(3) + "%",
                "opacity",
                1
              );
            }
          ...
          var t3 = fade.t3;
          var t4 = fade.t4;
          var keyTexts = [t1$2, t2$2, t3, t4].map(function (t) {
            return ((t / duration) * 100).toFixed(3) + "%";
          });
          var values$1 = [a1, a2, a3].map(function (a) {
            return 1 - a / 255;
          });
          diaKbl.set("0.000%", "opacity", values$1[0]);
          if (t1$2 < duration) {
            diaKbl.set(keyTexts[0], "opacity", values$1[0]);
          }
          if (t2$2 < duration) {
            diaKbl.set(keyTexts[1], "opacity", values$1[1]);
          }
          if (t3 < duration) {
            diaKbl.set(keyTexts[2], "opacity", values$1[1]);
          }
          if (t4 < duration) {
            diaKbl.set(keyTexts[3], "opacity", values$1[2]);
          }
      ...
      slices.forEach(function (slice) {
        slice.fragments.forEach(function (fragment) {
          ...
          mergeT(fragment.tag.t).forEach(function (ref) {
            ...
            if (tag.fs) {
              var from =
                this$1.scale * getRealFontSize(fromTag.fn, fromTag.fs) + "px";
              var to =
                this$1.scale * getRealFontSize(tag.fn, fromTag.fs) + "px";
              kbl.setT({
                t1: t1,
                t2: t2,
                duration: duration,
            ...
            if (tag.c1 || (tag.a1 && !hasAlpha)) {
              ...
              var to$2 = color2rgba(
                (tag.a1 || fromTag.a1) + (tag.c1 || fromTag.c1)
              );
              kbl.setT({
                t1: t1,
                t2: t2,
  ...
  function createDrawing(fragment, styleTag) {
    ...
    var strokeScale = /Yes/i.test(this.info.ScaledBorderAndShadow)
      ? this.scale
      : 1;
    ...
    slices.forEach(function (slice) {
      ...
      slice.fragments.forEach(function (fragment) {
        ...
        if (!drawing) {
          ...
          cssText +=
            "font-size:" +
            this$1.scale * getRealFontSize(tag.fn, tag.fs) +
            "px;";
          ...
          if (borderStyle === 3) {
            cssText +=
              "background-color:" +
              color2rgba(tag.a3 + tag.c3) +
              ";" +
              "box-shadow:" +
              createCSSStroke(tag, scale) +
              ";";
          }
          cssText += tag.b
            ? "font-weight:" + (tag.b === 1 ? "bold" : tag.b) + ";"
            : "";
        ...
        if (hasTransfrom) {
          cssText +=
            vendor.transform + "transform:" + createTransform(tag) + ";";
          if (!drawing) {
            cssText +=
              "transform-style:preserve-3d;word-break:break-word;white-space:normal;";
          }
        }
        if (animationName) {
  ...
  function allocate(dialogue) {
    ...
    var end = dialogue.end;
    var stageWidth =
      this.width - ((this.scale * (margin.left + margin.right)) | 0);
    ...
    if (alignment <= 3) {
      ...
      for (
        var i$2 = (stageHeight - height) >> 1;
        i$2 < stageHeight - vertical;
        i$2++
      ) {
        if (find(i$2)) {
          break;
  ...
  function getPosition(dialogue) {
    ...
    if (effect) {
      if (effect.name === "banner") {
        if (alignment <= 3) {
          y = this.height - height - margin.vertical;
        }
        if (alignment >= 4 && alignment <= 6) {
      ...
      if (alignment % 3 === 2) {
        x = this.scale * pos.x - width / 2;
      }
      if (alignment % 3 === 0) {
        x = this.scale * pos.x - width;
      }
      if (alignment <= 3) {
        y = this.scale * pos.y - height;
      }
      if (alignment >= 4 && alignment <= 6) {
        y = this.scale * pos.y - height / 2;
      }
      if (alignment >= 7) {
        y = this.scale * pos.y;
      }
      ...
      if (alignment % 3 === 0) {
        x = this.width - width - this.scale * margin.right;
      }
      var hasT = slices.some(function (slice) {
        return slice.fragments.some(function (ref) {
  ...
  function framing() {
    ...
    for (var i = this._.stagings.length - 1; i >= 0; i--) {
      ...
      if (dia.effect && /scroll/.test(dia.effect.name)) {
        ...
        var delay = ref.delay;
        var duration =
          ((y2 || this._.resampledRes.height) - y1) / (1000 / delay);
